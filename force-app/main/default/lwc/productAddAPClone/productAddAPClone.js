...
const COLS=[
    ...
    {label:'Rate',fieldName:'Kurl_On_to_Retailer_Price__c', type:'text','initialWidth': 150, sortable: true},
...
export default class ProductAddAPClone extends NavigationMixin(LightningElement) {
       ...
       // getGST({ lstProdId: event.detail.draftValues[0].Id,oppId:this.recordId,flag:flag}).then((result) => {
        ...
        getGST({ lstProdId: event.detail.draftValues[0].Id,oppId:this.recordId,flag:flag}).then((result) => {
           ...
           // gstPrice = gstPrice + this.products[0].Quantity*this.mrp*(gst/100);
            ...
            this.quantityEnd = quantTotal;
            //this.total = this.MRP_Rollup + this.gstPriceTotal;
            let priceAfterDisc = (this.mrp * Number(event.detail.draftValues[0].Quantity));
            this.basicValueMap.set(event.detail.draftValues[0].Id,priceAfterDisc * (1 - (this.additionalDiscount/100)));
    ...
    handleClick(event){
        ...
        let brandArr = [];
        for(let [key,value] of this.bovMap){
            brandArr = [...brandArr,value.Brand__c];
            console.log('BrndArray'+brandArr);
        }
            ...
            return;
              ...
              if((value.Brand__c == 'Layers' && flag == 2) ||(value.Brand__c == 'Boutique Living' && flag == 1) || flag == 0){
                ...
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Error',
                        message: 'The Sales Order have the Products for Brand" '+this.brandCheck+' "You cannot add Mixed Brands',
                        variant: 'error'
                    })
                );
                return;
