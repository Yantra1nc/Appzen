...
import getProductsDynamicDisc from '@salesforce/apex/DynamicDiscount.getProductsDynamicDisc';
import getProductCount from "@salesforce/apex/DynamicDiscount.getCountOfRecords";
...
import getProductDetails from "@salesforce/apex/DynamicDiscount.getProductDetails";
import getProductMrp from "@salesforce/apex/DynamicDiscount.getProductMrp";
import {
    refreshApex
} from '@salesforce/apex';
import getOppLineItems from '@salesforce/apex/DynamicDiscount.getOppLineItems';
...
/*{
    ...
    fieldName: 'Additional_Discount__c',
    type: 'text',
    editable: 'true',
    'wrapText': true,
    cellAttributes: {
...
export default class AddEditTestComp extends NavigationMixin(LightningElement) {
    @api recordId;
    cols = COLS;
    searchKey = "";
    subscription = null;
    ...
    wiredProducts(result) {
        ...
        if (result.data) {
            ...
            if (this.bovMap.size > 0) {
                console.log('Bov Map contains products');
                this.page = 1;
                this.items = result.data;
                ...
                this.updateValues();
                console.log(this.deleteMap.size);
                ...
                if (this.filtersData.collection.length == 0 && this.filtersData.designType.length == 0 && this.filtersData.brand.length == 0 && this.filtersData.size.length == 0 && this.filtersData.color.length == 0 && this.filtersData.minPrice == 0 && this.filtersData.maxPrice == 10000) {
                    for (let [key, value] of this.bovMap) {
                        this.productsArray.push({
                            ...
                            Kurl_On_to_Retailer_Price_1__c: value.Kurl_On_to_Retailer_Price_1__c,
                    ...
                    console.log('Loadmore..filters not null');
                    this.loadData();
                }
                ...
                getOppLineItems({
                    ...
                    if (result.length > 0) {
                        result.forEach(element => {
                            ...
                            console.log(element.Product2.Kurl_On_to_Retailer_Price_1__c);
                            if (element.Additional_Discount__c === undefined) {
                                element.Additional_Discount__c = 0;
                            }
                            calculateDynamicDiscount({productId:element.Product2Id,oppId:this.recordId})
                            .then((res) => {
                                let gst = 0;
                                let gstFinal = 0;
                                let gstPriceSum = 0;
                                if (res == 0) {
                                    res = element.Dynamic_Discount__c;
                                }
                                ...
                                console.log('res '+ res);
                                console.log('element.GST__c '+ element.GST_in_1__c);
                                ...
                                gstPrice = element.Quantity * res * (element.GST_in_1__c / 100);
                                ...
                                let sumGst = 0;
                                console.log('gstMap '+ this.gstMap);
                                ...
                                let priceAfterDisc = (Number(res) * Number(element.Quantity));
                                ...
                                let finalPrice = (Number(element.Quantity) * Number(res)) + gstFinal;
                                var sumDiscount = 0;
                                //This array is the list of opportunity line items to be displayed at the top when component loads
                                arr1 = [...arr1, {
                                    ...
                                    Kurl_On_to_Retailer_Price_1__c: element.Product2.Kurl_On_to_Retailer_Price_1__c,
                                ...
                                console.log('Array1 '+JSON.stringify(arr1));                               
                                ...
                                //This array contains the rest of filtered data
                                for (let index = 0; index < this.products.length; index++) {
                                    ...
                                    arr2 = [...arr2, {
                                        ...
                                        Name: ele.Name,
                                        Size__c: ele.Size__c,
                                        Design_Num__c: ele.Design_Num__c,
                                        Kurl_On_to_Retailer_Price__c: ele.Kurl_On_to_Retailer_Price__c,
                                        Kurl_On_to_Retailer_Price_1__c: ele.Kurl_On_to_Retailer_Price_1__c,
                                ...
                                //Concatenate arr1 and arr2 and eliminate the duplicate products and storing in result array.  
                                console.log('arr1 '+JSON.stringify(arr1));
                               // console.log('arr2 '+JSON.stringify(arr2));
                                this.resultArr = [...new Set(arr1.concat(arr2))];
                                //console.log('Line 463' + JSON.stringify(this.resultArr));
                        ...
                        console.log('Have lineitems...bovMap contains products');
                ...
                if (result) {
                    ...
                    result.forEach(element => {
                        this.loadProdArray.push({
                            ...
                            Kurl_On_to_Retailer_Price_1__c: element.Kurl_On_to_Retailer_Price_1__c,
                    ...
                    console.log('BovMap '+this.bovMap);
    ...
    //Whenever quantity is entered this method gets called
    handleChange(event) {
        var selectedRows=event.detail.selectedRows;
        console.log('Draft row-> ' + JSON.stringify(selectedRows));
        console.log('Draft-> ' + JSON.stringify(event.detail.draftValues[0]));
        ...
        if (matched) {
            ...
            //Call apex method to calculate the dynamic discount based on products
            calculateDynamicDiscount({productId:event.detail.draftValues[0].Id,oppId:this.recordId})
            .then((res) => {
                console.log('Dynamic Discount '+res);
                //this.discount = res;
                getProductMrp({
                    prodId: event.detail.draftValues[0].Id,
                    recordId:this.recordId
                }).
                    then((result) => {
                        console.log(result);
                        ...
                        if (res == 0) {
                            this.mrp = result[0].Kurl_On_to_Retailer_Price__c;
                        }else{
                            this.mrp = res;
                        }
                        //this.mrp = result[0].Kurl_On_to_Retailer_Price__c;
                        ...
                        this.productImage = result[0].Product_Image_Link__c;
                        this.brand = result[0].Brand__c;
                        this.collection = result[0].Collection__c;
                        this.prodName = result[0].Name;
                        this.size = result[0].Size__c;
                        this.designNo = result[0].Design_Num__c;
                        this.color = result[0].Color__c;
                        this.availability = result[0].Availability__c;
                        this.mop = result[0].MOP__c;
                        ...
                        getGST({
                            lstProdId: event.detail.draftValues[0].Id,
                            oppId: this.recordId,
                            flag: flag
                        }).then((result) => {
                        ...
                        this.oppLineItemRec.push({
                            ...
                            Kurl_On_to_Retailer_Price__c: result[0].Kurl_On_to_Retailer_Price__c,
                            Kurl_On_to_Retailer_Price_1__c: result[0].Kurl_On_to_Retailer_Price_1__c,
                            Color__c: this.color,
                            //Design_Type__c:this.design,
    ...
    async getDynamicDiscount(){
        calculateDynamicDiscount({productId:event.detail.draftValues[0].Id,oppId:this.recordId})
            .then((result) => {
                console.log('Dynamic Discount '+result);
            }).catch((err) => {
                console.log(err);
            });
    }
    ...
    handleClick(event) { 
        ...
        let brandArr = [];
        let isWhole = true;
        console.log('Bov Map '+this.bovMap.size);
        for (let [key, value] of this.bovMap) {
            brandArr = [...brandArr, {
                name: value.Name,
        ...
        if (this.bovMap.size > 0) {
            for (let [key, value] of this.bovMap) {   
                ...
                console.log('Inside bovmap loop'+this.idsToDelete);
                let exMill = (Number(value.Final_Price__c)+Number(this.gstMap.get(value.Id)))/value.Quantity;
                console.log('exMill '+exMill);
                if (exMill > 1000) {
                    flag = false;
                }else{
                    flag = true;
                }
                getGstWrapper({lstProdId:value.Id,oppId:this.recordId,flag:flag
                }).then((getWrapRes) => {                    
                    console.log('gstWrpper' + JSON.stringify(getWrapRes));
            ...
            console.log('Save Items after loop '+ JSON.stringify(this.saveOppLineItems));
            
            console.log('Length' + this.saveOppLineItems.length);
        }else if(this.bovMap.size == 0){
    ...
    handleRowAction(event) {
        ...
        console.log('After delete '+JSON.stringify(this.bovMap));
        ...
        this.deleteItemArr.push({
            ...
            Kurl_On_to_Retailer_Price_1__c: row.Kurl_On_to_Retailer_Price_1__c,
