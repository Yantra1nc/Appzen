   ...
   // this.MRP_Rollup = this.MRP_Rollup + finalPrice; 
    ...
    handleClick(event){
        ...
        for (let [key, value] of this.bovMap) { 
           ...
           console.log('ID'+this.recordId);
           value.Final_Price__c


        
                
                //Call apex method to calculate the dynamic discount based on products
            calculateDynamicDiscount({productId:value.Id,oppId:this.recordId}).then((res) => {
                console.log('***** response ->  ',res);
               //get GST 
               getGstWrapper({lstProdId:value.Id,oppId:this.recordId,flag:flag}).then((getWrapRes) => {
                //get data here and update
                    getPriceMaster({productId:value.Id,oppId:this.recordId}).then((pm) => {
                        this.spinner = true;
                        ...
                        if(count > 0){
                            ...
                            console.log('-> PM ERROR ',JSON.stringify(err.message));
                            var msg = JSON.stringify(err.body.message);
                            this.spinner = false;
                            if(msg.includes("ENTITY_IS_LOCKED")){
                        ...
                        console.log('DISC ERROR : ',err);
                    });
                }).catch((err) => {
                    this.spinner = false;

                        console.log('-> cal culetion error ',JSON.stringify(err.message));   
                        var msg = JSON.stringify(err.body.message);
                        this.spinner = false;
                        if(msg.includes("ENTITY_IS_LOCKED")){
                        msg = 'You can not Update/Edit items.';
